...
public class Library {
    ...
    private boolean extraCredit, restart;

    public Library(){
        
    }
    
    public Library(boolean xtraCred) {
    ...
    /**
     * This method calls all necessary methods and prompts the user
     * for the information needed for the program to function.
     */
    public void run() {
        String book;
        boolean resume = true;
     ...
     * @return A boolean value that indicates the end of a loop in the run() method.
     */
    public boolean process() {
        for (Book bk : bookObjects) {
            bk.run();
            features.add(new Features(bk.getWords(), bk.getSentences(), extraCredit));
        }
        int i = 0;
        ...
        writer = new Writer(author, booksFeatures, numBooks, extraCredit);
        writer.run();
    ...
    public void print() {
        ...
        for (int i = 0; i < numBooks; i++) {
            ...
            if(extraCredit){
                System.out.printf("| %25s %9.6f |%n", "Punctuation Frequency: ", booksFeatures.get(i).get(5));
                System.out.printf("| %25s %9.6f |%n", "Common Word Frequency: ", booksFeatures.get(i).get(6));
            }
            System.out.println("---------------------------------------");
        }
        ...
        if(extraCredit){
            System.out.printf("| %25s %9.6f |%n", "Punctuation Frequency: ", authorFeatures.get(5));
            System.out.printf("| %25s %9.6f |%n", "Common Word Frequency: ", authorFeatures.get(6));
        }
        System.out.println("---------------------------------------");
    }
}
